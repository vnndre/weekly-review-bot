...
Drop a CSV into a watched folder → a Markdown report lands in the right period folder a minute later.
...
3. **Parses the CSV in-memory** and computes:
   ...
   - Auto-detected staff name mentions
...
22 nodes. Deterministic math in code, LLM only for narrative — the model never touches the numbers.
...
### Name Drops
[Staff members mentioned positively by name]
```
...
4. In every Google Drive node with `YOUR_GOOGLE_DRIVE_FOLDER_ID` placeholder, set the appropriate folder (root report folder, archive folder)
5. Create an empty `metrics_history.json` file in the same Drive folder the trigger watches, containing: `{"weeks": []}`
...
- **Name detection is lossy by design.** It surfaces any capitalized 3+ letter word it hasn't seen in the stopword list; the human reviewing the report is expected to filter false positives. Better to surface too many names than miss one.
